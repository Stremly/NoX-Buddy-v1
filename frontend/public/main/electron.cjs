...
function startNoxBackend() {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (isDev) {
        // Dev mode: Python files in ../../backend (relative to frontend/public/main)
        backendPath = path.join(__dirname, '..', '..', '..', 'backend');
        pythonCommand = process.platform === 'win32' ? 'python' : 'python3';
        mainScript = 'main.py';
      } else {
        // Prod mode: Python files bundled in resources folder
        backendPath = path.join(process.resourcesPath, 'backend');
        pythonCommand = process.platform === 'win32' ? 'python' : 'python3';
        mainScript = 'main.py';
      }

      const mainPyPath = path.join(backendPath, mainScript);
      ...
      // Spawn Python process to run the backend
      noxBackendProcess = spawn(pythonCommand, [mainScript], {
        cwd: backendPath,
      ...
      console.log(`🚀 Backend started with Python: ${pythonCommand} ${mainScript}`);
