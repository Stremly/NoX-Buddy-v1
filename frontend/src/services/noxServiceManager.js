...
class NoxServiceManager {
  ...
  initializeService() {
    ...
    if (isElectron && isWindows) {
      // Best case: Electron on Windows - use real backend
      ...
      console.log('🔧 Backend executable will be loaded from backend/ directory');
    } else if (isElectron && (isMac || isLinux)) {
      // Electron on Mac/Linux - backend won't work, use mock
      this.activeService = mockNoxService;
      this.serviceType = 'electron-unix';
      console.log('⚠️ Using Mock Nox Service (macOS/Linux - Demo Mode)');
      console.log('💡 For full features, use Windows version');
    } else {
  ...
  getStatusMessage() {
    switch (this.serviceType) {
      case 'electron-windows':
        return 'Nox Backend running with full capabilities';
      case 'electron-unix':
        return 'Demo mode: Nox Backend is designed for Windows systems';
      case 'browser':
  ...
  async startBackend() {
    try {
      ...
      if (this.serviceType === 'electron-windows') {
  ...
  getPlatformMessage() {
    ...
    switch (this.serviceType) {
      case 'electron-windows':
        ...
        break;
      case 'electron-unix':
        message = '💻 You\'re on macOS/Linux. Nox Backend is currently Windows-only, so you\'re seeing our demo mode.';
        break;
      case 'browser':
