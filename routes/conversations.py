...
def check_contact(nox_id: str, peer_id: str, secret_code: str):
    """Ensure peer is in contacts and user is authorized."""
    user = db.users.find_one({"nox_id": nox_id, "secret_code": secret_code})
    if not user:
...
def start_conversation(nox_id: str, peer_id: str, secret_code: str = Query(...)):
    check_contact(nox_id, peer_id, secret_code)
...
def add_message(nox_id: str, peer_id: str, message: str, secret_code: str = Query(...)):
    check_contact(nox_id, peer_id, secret_code)
...
def get_conversation(nox_id: str, peer_id: str, secret_code: str = Query(...)):
    check_contact(nox_id, peer_id, secret_code)
...
def delete_message(nox_id: str, peer_id: str, message_id: str, secret_code: str = Query(...)):
    check_contact(nox_id, peer_id, secret_code)
...
def delete_conversation(nox_id: str, peer_id: str, secret_code: str = Query(...)):
    check_contact(nox_id, peer_id, secret_code)

    for nid, pid in [(nox_id, peer_id), (peer_id, nox_id)]:
        db.nox.update_one({"nox_id": nid}, {"$unset": {f"conversations.{pid}": ""}})
