...
function startNoxBackend() {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (isDev) {
        ...
        // Production: relative to app resources (you'll need to bundle Python for production)
        backendPath = path.join(process.resourcesPath, 'backend');
      }
      
      console.log('Backend path:', backendPath);
      
      // Check if backend directory exists and has main.py
      const fs = require('fs');
      ...
      if (!fs.existsSync(mainPyPath)) {
        throw new Error(`Backend main.py not found at: ${mainPyPath}`);
      }

      // Start the Python backend process
      noxBackendProcess = spawn('python', ['main.py'], {
      ...
      noxBackendProcess.stdout.on('data', (data) => {
        ...
        if (output.includes('Uvicorn running on') && !backendStarted) {
          backendStarted = true;
          ...
          if (mainWindow) {
            mainWindow.webContents.send('nox-status', { 
              running: true, 
              message: 'Python Backend started on http://localhost:8000' 
            });
          }
          resolve({ success: true, url: 'http://localhost:8000' });
        }
        
        // Forward all output to renderer
        if (mainWindow) {
  ...
  if (!backendStarted) {
    ...
    isBackendStarting = false;
    resolve({ success: true, url: 'http://localhost:8000', warning: 'Startup timeout but continuing' });
  }
}, 15000);
      ...
      startupTimeout = setTimeout(() => {
        if (!backendStarted) {
          ...
          reject({ success: false, error: 'Backend startup timeout - check if Python dependencies are installed' });
        }
      }, 10000);
